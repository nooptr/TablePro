...
<Tip>
You can also connect by opening a `postgresql://` URL from your browser or terminal. See [Connection URL Reference](/databases/connection-urls) for details.
</Tip>
...
Switch between PostgreSQL databases and schemas with the database switcher (**Cmd+K**):

1. Press **Cmd+K** to open the switcher. It lists databases for PostgreSQL connections
...
Execute queries with full PostgreSQL syntax:
...
TablePro renders PostgreSQL's rich type system correctly:
...
For **Verify CA** and **Verify Identity** modes, provide the path to your CA certificate (root certificate). You can also provide optional client certificate and key files for mutual TLS.

<Tip>
Cloud providers like AWS RDS, Heroku, and Supabase typically require SSL. Use **Required** or **Verify CA** depending on your security needs.
</Tip>
...
Set session variables that apply automatically on every connection. Configure these in the **Advanced** tab of the connection form.

Typical PostgreSQL startup commands:
...
Setting `search_path` is useful when your tables live in a non-default schema. Instead of qualifying every table name (`myschema.users`), set the search path once and query tables by name directly.

Setting `timezone` ensures all `timestamptz` queries return results in the same zone, regardless of the server or client OS timezone.

See [Startup Commands](/databases/overview#startup-commands) for more details.

## ~/.pgpass File Support

TablePro supports the standard PostgreSQL `~/.pgpass` file for password-less authentication. This is useful for environments where passwords are managed by external secrets tools.

### Setup

1. Create `~/.pgpass` with entries in the format: `hostname:port:database:username:password`
2. Set permissions: `chmod 0600 ~/.pgpass`
3. In the connection form, enable **Use ~/.pgpass** in the Authentication section
4. Leave the password field blank

### Format

Each line in `~/.pgpass` follows this format:
...
```
# Match any database on localhost for user postgres
localhost:5432:*:postgres:mypassword

# Match any host/port/database for a specific user
*:*:*:deploy_user:deploy_password
```

Escape literal colons with `\:` and backslashes with `\\`.

### Validation

The connection form shows live status when **Use ~/.pgpass** is enabled:

- **Green**: `~/.pgpass` found with a matching entry
- **Yellow**: File not found, or no matching entry
- **Red**: File has incorrect permissions (needs `chmod 0600`)

<Warning>
libpq silently ignores `~/.pgpass` if file permissions are not exactly `0600`. TablePro surfaces this as a clear warning in the connection form.
</Warning>

<Note>
When using SSH tunneling, libpq connects to `127.0.0.1` on the local tunnel port, not the remote host. Use wildcards (`*`) for host and port in your `~/.pgpass` entries to match tunneled connections.
</Note>

## Pre-Connect Script

Run a shell script before each connection attempt. This is useful for refreshing credentials, updating `~/.pgpass`, or performing pre-flight checks.

### Setup

1. Open the **Advanced** tab in the connection form
2. Enter your script in the **Pre-Connect Script** field
3. The script runs via `/bin/bash -c` before each connection

### Behavior

- The script inherits the parent environment (HOME, PATH, etc.)
- Non-zero exit code aborts the connection with an error message
- 10-second timeout: the script is terminated if it takes too long
- Only runs on explicit connect, not on automatic reconnections

### Example: Refresh pgpass from secrets manager

```bash
#!/bin/bash
vault read -field=password secret/postgres > ~/.pgpass_entry
echo "db.example.com:5432:mydb:app_user:$(cat ~/.pgpass_entry)" > ~/.pgpass
chmod 0600 ~/.pgpass
rm ~/.pgpass_entry
```
