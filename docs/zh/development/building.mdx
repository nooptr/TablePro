...
TablePro 使用原生 C 库进行数据库连接。这些库托管在专用的 [GitHub Release](https://github.com/datlechin/TablePro/releases/tag/libs-v1) 上，通过 `scripts/download-libs.sh` 下载。
...
构建脚本（`build-release.sh`）会自动从 universal 库中提取正确架构的切片。
