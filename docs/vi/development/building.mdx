...
TablePro dùng thư viện C native cho kết nối database. Các thư viện được lưu trữ trên [GitHub Release](https://github.com/datlechin/TablePro/releases/tag/libs-v1) riêng và tải về qua `scripts/download-libs.sh`.

### Thiết lập thư viện

```bash
# Tải thư viện (bỏ qua nếu đã có)
...
Build script (`build-release.sh`) tự động trích slice kiến trúc đúng từ thư viện universal.
