...
struct CellDisplayFormatterTests {
    ...
    func emptyString() {
        let result = CellDisplayFormatter.format("", columnType: nil)
        #expect(result == "")
    }

    @Test("plain text passes through unchanged")
    func plainTextPassthrough() {
        let result = CellDisplayFormatter.format("hello world", columnType: nil)
        #expect(result == "hello world")
    }

    @Test("text with linebreaks is sanitized")
    func linebreaksSanitized() {
        let result = CellDisplayFormatter.format("line1\nline2\rline3", columnType: nil)
    ...
    func longTextTruncated() {
        ...
        let result = CellDisplayFormatter.format(longString, columnType: nil)
    ...
    func exactMaxLengthNotTruncated() {
        ...
        let result = CellDisplayFormatter.format(exactString, columnType: nil)
        #expect(result == exactString)
    }

    @Test("nil column type skips type-specific formatting")
    func nilColumnType() {
        let result = CellDisplayFormatter.format("2024-01-01", columnType: nil)
